...
class Post {
		...
		if($check_empty != "") {
			...
			foreach($body_array as $key => $value) {
				...
				if(strpos($value, "www.youtube.com/watch?v=") !== false) {
					...
					// if link targets a playlist on youtube:
					// https://www.youtube.com/watch?v=n3ICr6dnKRw&index=4&list=PL0lO_mIqDDFXm69bqj5JTCS1XGTNkhTch
					// first element https://www.youtube.com/watch?v=n3ICr6dnKRw
					// second element index=4
					// third element list=PL0lO_mIqDDFXm69bqj5JTCS1XGTNkhTch
					$link = preg_split("!&!", $value);
					$value = preg_replace("!watch\?v=!", "embed/", $link[0]); // we need only the first element
	...
	public function loadPostsFriends($data, $limit) {
		...
		//read posts when post is not delete 
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				?>
				<script>
					//this is message and alert for delete post 
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				$date_time = $row['date_added'];
					...
					//display pichture , first name, last name 
					$str .= "<div class='status_post' onClick='javascript:toggle$id(event)'>
								<div class='post_profile_pic'>
									<img src='$profile_pic' width='50'>
								</div>
